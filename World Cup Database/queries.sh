...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams RIGHT JOIN games ON teams.team_id = games.winner_id  ORDER BY name")" #it is right join, because full join causes only the teams table to show and shows every team. Right join allows you to see the winning team
